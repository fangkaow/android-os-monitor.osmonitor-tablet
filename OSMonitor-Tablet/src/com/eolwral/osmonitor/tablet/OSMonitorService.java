	...
	private Runnable mRefresh = new Runnable() 
	{
		@Override  
            public void run() {
				...
				String maininfo = serviceContext.getResources().getString(R.string.process_usage_title)+cpuLoad+"% , "+
									serviceContext.getResources().getString(R.string.process_memory_title)+":"+MemoryFormat.format(JNILibrary.GetMemBuffer()+JNILibrary.GetMemCached()+JNILibrary.GetMemFree())+ "K"; 

				String extendinfo = "";
				if(useCelsius)
					extendinfo = serviceContext.getResources().getString(R.string.misc_battery_title)+": "+battLevel+"%"+" ("+temperature/10+"°C)";
				else
					extendinfo = serviceContext.getResources().getString(R.string.misc_battery_title)+": "+battLevel+"%"+" ("+((int)temperature/10*9/5+32)+"°F)";
...
/*				serviceNotify.contentView.setTextViewText(R.id.StatusBarCPU, "CPU: "+cpuLoad+"%");
	...
	*/			
    ...
    private void Enable()
    {
		...
		UpdateInterval = settings.getInt(Preferences.PREF_UPDATE, 2);
		JNILibrary.doDataTime(UpdateInterval);
